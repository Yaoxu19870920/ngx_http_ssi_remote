...
static inline ngx_int_t ngx_http_get_buf(u_char *buf, unsigned int len, ngx_chain_t **in, u_char **pos, ngx_log_t *log);

static inline ngx_int_t ngx_http_get_buf_test(u_char *buf, unsigned int len, u_char **pos, ngx_log_t *log);

static inline ngx_int_t ngx_http_get_buf_infile(u_char *buf, unsigned int len, ngx_chain_t *in);
...
static ngx_command_t  ngx_http_ssi_filter_commands[] = {
	...
	{ ngx_string("ssi_router_keys"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_ANY,
	  ngx_conf_set_str_array_slot,
	  NGX_HTTP_LOC_CONF_OFFSET,
	  offsetof(ngx_http_ssi_loc_conf_t, router_keys),
	  NULL },

	{ ngx_string("ssi_router_default"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_ANY,
	  ngx_conf_set_str_slot,
	  NGX_HTTP_LOC_CONF_OFFSET,
	  offsetof(ngx_http_ssi_loc_conf_t, router_default),
	  NULL },

	{ ngx_string("ssi_optimize"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_ANY,
	  ngx_conf_set_flag_slot,
	  NGX_HTTP_LOC_CONF_OFFSET,
	  offsetof(ngx_http_ssi_loc_conf_t, optimize),
	  NULL },
...
ngx_http_ssi_header_filter(ngx_http_request_t *r)
{
	...
	{
		ctx->ssi_found = 0;
		ctx->ssi_content_map = ngx_array_create(r->pool, 32, sizeof(ngx_request_chain_t));
		if(!(slcf->router_keys) && (slcf->router_default.len == 0))
		{
			ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ssi_router_keys or ssi_router_default or both shoule be in conf %s:%d\n", __FILE__, __LINE__);
			return NGX_ERROR;
		}
		
		ngx_uint_t loop;
		if(slcf->router_default.len == 0)
		{
			//ssi_router_default没有被设置，ssi_router_keys必定被设置了
			ctx->ssi_map = (ngx_keyval_t **)ngx_pcalloc(r->pool, (slcf->router_keys->nelts) * sizeof(ngx_keyval_t *));
			if(ctx->ssi_map == NULL)
			{
				return NGX_ERROR;
			}
			ngx_str_t *paths = slcf->router_keys->elts;
			for(loop = 0; loop < slcf->router_keys->nelts; ++loop)
			{
				ngx_keyval_t *kv = (ngx_keyval_t *)ngx_pcalloc(r->pool, sizeof(ngx_keyval_t));
				if((kv == NULL) || (&ctx->ssi_map[loop] == NULL))
				{
					return NGX_ERROR;
				}
				kv->key = paths[loop];
			...
			ctx->ssi_server_sum = slcf->router_keys->nelts;
			ctx->ssi_server_nodefault_sum = slcf->router_keys->nelts;
		}
		else
		{
			//ssi_router_default被设置了，需要判断ssi_router_keys有没有被设置
			ngx_str_t default_value = slcf->router_default;
			if(slcf->router_keys)
			{
				//ssi_router_keys被设置了
				ctx->ssi_map = (ngx_keyval_t **)ngx_pcalloc(r->pool, (slcf->router_keys->nelts + 1) * sizeof(ngx_keyval_t *));
				if(ctx->ssi_map == NULL)
				{
					return NGX_ERROR;
				}
				...
				ngx_keyval_t *kv = (ngx_keyval_t *)ngx_pcalloc(r->pool, sizeof(ngx_keyval_t));
				if((kv == NULL) || (&ctx->ssi_map[slcf->router_keys->nelts] == NULL))
				{
					return NGX_ERROR;
				}
				kv->key = default_value;
				ngx_str_set(&kv->value, "");
				ctx->ssi_map[slcf->router_keys->nelts] = kv;
				ctx->ssi_server_sum = slcf->router_keys->nelts + 1;
				ctx->ssi_server_nodefault_sum = slcf->router_keys->nelts;
			}
			else
			{
				//ssi_router_keys没有被设置了
				ctx->ssi_map = (ngx_keyval_t **)ngx_pcalloc(r->pool, sizeof(ngx_keyval_t *));
				ngx_keyval_t *kv = (ngx_keyval_t *)ngx_pcalloc(r->pool, sizeof(ngx_keyval_t));
				if((kv == NULL) || (&ctx->ssi_map[0] == NULL))
				{
					return NGX_ERROR;
				}
				kv->key = default_value;
				ngx_str_set(&kv->value, "");
				ctx->ssi_map[0] = kv;
				ctx->ssi_server_sum = 1;
				ctx->ssi_server_nodefault_sum = 0;
			}
		}
		ctx->ssi_content_map_temp = (ngx_ssi_content_t **)ngx_pcalloc(r->pool, ctx->ssi_server_sum * sizeof(ngx_ssi_content_t *));
		if(ctx->ssi_content_map_temp == NULL)
		{
			return NGX_ERROR;
		}
		for(loop = 0; loop < ctx->ssi_server_sum; ++loop)
		{
			ngx_ssi_content_t *kv = (ngx_ssi_content_t *)ngx_pcalloc(r->pool, sizeof(ngx_ssi_content_t));
			if((kv == NULL) || (&ctx->ssi_content_map_temp[loop] == NULL))
			{
				return NGX_ERROR;
			}
			kv->key = ctx->ssi_map[loop]->key;
			kv->value = (ngx_chain_t **)ngx_pcalloc(r->pool, 200 * sizeof(ngx_chain_t*));;
			ctx->ssi_content_map_temp[loop] = kv;
		}
	}
    return ngx_http_next_header_filter(r);
}
...
ngx_http_get_buf_infile(u_char *buf, unsigned int len, ngx_chain_t *in)
{
	int n = ngx_read_file(in->buf->file, buf, len, 0);
	if(n == -1)
	{
		return NGX_ERROR;
	}
...
static inline ngx_int_t
ngx_http_get_buf(u_char *buf, unsigned int len, ngx_chain_t **in, u_char **pos, ngx_log_t *log)
{
	if (*in == NULL)
	{
		ngx_log_error(NGX_LOG_ERR, log, 0, "tag at %s:%d\n", __FILE__, __LINE__);
		return NGX_ERROR;
	}
	u_char *cpy = buf;
	u_char *rpos = *pos;
	...
	if ((*in)->buf == NULL || rpos == (*in)->buf->last)
	{
		*in = (*in)->next;
		if(*in == NULL)
		{
	...
	length = (*in)->buf->last-rpos >= len? len: (*in)->buf->last-rpos;
	while (len > 0) 
	{
		memcpy(cpy, rpos, length);
		len -= length;
		cpy += length;
		rpos += length;
		if (len > 0)
...
static inline ngx_int_t
ngx_http_get_buf_test(u_char *buf, unsigned int len, u_char **pos, ngx_log_t *log)
{
	u_char *rpos = *pos;
...
static ngx_int_t 
subrequest_handler(ngx_http_request_t *r, void *data, ngx_int_t rc)
{
	int ret;
	int count = 0;
	u_char len_buf[sizeof(unsigned int)] = {0};	
	u_char *file_name_buf;
	u_char *file_content_buf = NULL;
	unsigned int filename_len;
	unsigned int file_content_len = 0;
	unsigned int loop;
	ngx_chain_t **fragment_in_sub = NULL;
	...
	for(loop = 0; loop < mctx->ssi_content_map->nelts; ++loop)
	{
		...
		{
			fragment_in_sub = kv_temp[loop].value;
			break;
		}
	}
	if(!fragment_in_sub)
	...
	ngx_chain_t *file_2_memory = r->postponed->out;
	while(file_2_memory)
	{
		if(file_2_memory->buf->in_file == 1)
		{
			u_char *buf = (u_char *)ngx_pcalloc(r->main->pool, file_2_memory->buf->file_last * sizeof(u_char));
			if(buf == NULL)
			{
				return NGX_ERROR;
			}
			if(NGX_OK != ngx_http_get_buf_infile(buf, file_2_memory->buf->file_last, file_2_memory))
			{
				ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_http_get_buf_infile error at %s:%d\n", __FILE__, __LINE__);
				return NGX_ERROR;
			}
		...
		(r->postponed->out->next && (r->postponed->out->next->buf->pos < r->postponed->out->next->buf->last)))
	{
		...
		if(NGX_ERROR == ret)
		{
			ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_http_get_buf error at %s:%d\n", __FILE__, __LINE__);
			return NGX_ERROR;
		}
		for (filename_len = 0, loop = 0; loop < sizeof(filename_len); loop++) 
		{
			filename_len |= ((len_buf[loop] & 0xff) << (loop*8));
		}
		//获取文件名
		file_name_buf = (u_char *)ngx_pcalloc(r->pool, filename_len * sizeof(u_char));
		if(file_name_buf == NULL)
		{
			return NGX_ERROR;
		}
		ret = ngx_http_get_buf(file_name_buf, filename_len, &r->postponed->out, &cur_pos, r->connection->log);
		if(NGX_ERROR == ret)
		{
			ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "filename_len = %d at %s:%d\n", filename_len, __FILE__, __LINE__);
			return NGX_ERROR;
		}
		//获取文件长度
		ret = ngx_http_get_buf(len_buf, 4, &r->postponed->out, &cur_pos, r->connection->log);
		...
		for (file_content_len = 0, loop = 0; loop < sizeof(file_content_len); loop++) 
		{
			file_content_len |= ((len_buf[loop] & 0xff) << (loop*8));
		}
		//获取文件内容
		file_content_buf = (u_char *)ngx_pcalloc(r->pool, file_content_len * sizeof(u_char));
		if(file_content_buf == NULL)
		{
			return NGX_ERROR;
		}
		ret = ngx_http_get_buf(file_content_buf, file_content_len, &r->postponed->out, &cur_pos, r->connection->log);
		if(NGX_ERROR == ret)
		{
			ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_http_get_buf error at %s:%d\n", __FILE__, __LINE__);
			return NGX_ERROR;
	...
	/*ngx_chain_t *chain_temp_last_buf = mctx->ssi_content;
	while(chain_temp_last_buf->next->next)
	{
		chain_temp_last_buf = chain_temp_last_buf->next;
	}
	chain_temp_last_buf->next = NULL;
	chain_temp_last_buf->buf->last_buf = 1;
	chain_temp_last_buf->buf->last_in_chain = 1;*/
	...
	while(mctx->ssi_content)
	{
		...
		if((chain_temp->buf->pos == 0) && (chain_temp->buf->last == 0))
		{
			break;
		}
...
ngx_http_ssi_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
            ...
            && ctx->busy == NULL))
    {
	    if(r->parent)
		{
			ngx_http_ssi_loc_conf_t *slcf_pr = ngx_http_get_module_loc_conf(r->parent, ngx_http_ssi_filter_module);
			if(slcf_pr->optimize)
			{
            ...
            if (rc == NGX_OK) {
                ...
                if (cmd->flush && ctx->out) {
                    ...
                    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                                   "ssi flush");
					if(0 == memcmp(cmd->name.data, "include", 7))
					{
						mctx->ssi_found = 1;
					}
	...
	//=============================================================================
	int ret = ngx_http_ssi_output(r, ctx);
	ngx_http_request_t *sr = NULL;
	ngx_str_t new_args;
	ngx_uint_t flags = 0;
	flags |= NGX_HTTP_LOG_UNSAFE;

	for(i = 0; i < ctx->ssi_server_sum; ++i)
	{
		if(0 == ctx->ssi_map[i]->value.len)
		{
			continue;
		}

		new_args.data = ngx_pcalloc(r->pool, ctx->ssi_map[i]->value.len + 1);
		if (new_args.data == NULL)
		{
			return NGX_ERROR;
		}
		memcpy(new_args.data, "?", 1);
		memcpy(new_args.data + 1, ctx->ssi_map[i]->value.data, ctx->ssi_map[i]->value.len);
		new_args.len = ctx->ssi_map[i]->value.len + 1;
		...
		if (ngx_http_subrequest(r, &ctx->ssi_map[i]->key, &new_args, &sr, psr, flags) != NGX_OK)
		{
   			return NGX_HTTP_SSI_ERROR;
		}
		ctx->ssi_sub_count++;
		ctx->ssi_map[i]->value.data = NULL;
		ctx->ssi_map[i]->value.len = 0;

		ngx_request_chain_t *kv_temp = ngx_array_push(ctx->ssi_content_map);
		kv_temp->key = sr;
		kv_temp->value = ctx->ssi_content_map_temp[i]->value;
	}
	//=============================================================================
...
ngx_http_ssi_output(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx)
{
	...
	ngx_http_ssi_loc_conf_t  *slcf = ngx_http_get_module_loc_conf(r, ngx_http_ssi_filter_module);
	ngx_http_ssi_ctx_t *mctx = ngx_http_get_module_ctx(r->main, ngx_http_ssi_filter_module);
    ...
    ngx_str_t **params)
{
	...
	//=================================================================
	//ssi相关的其他include指令可能支持的不好
	int loop;
	unsigned fragment_in_default = 1;
	u_char *buf_4_subrequest = (u_char *)ngx_pcalloc(r->main->pool, 16384 * sizeof(u_char));
	if(buf_4_subrequest == NULL)
	{
		return NGX_ERROR;
	}

	ngx_chain_t *in = (ngx_chain_t *)ngx_pcalloc(r->main->pool, sizeof(ngx_chain_t));
	if(in == NULL)
	{
		return NGX_ERROR;
	}
	in->buf = (ngx_buf_t *)ngx_pcalloc(r->main->pool, sizeof(ngx_buf_t));
	if(in->buf == NULL)
	{
		return NGX_ERROR;
	}
	...
	//在ssi_content中留个位置
	if (ngx_chain_add_copy(r->main->pool, &ctx->ssi_content, in) != NGX_OK) 
	{
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_chain_add_copy error at %s:%d\n", __FILE__, __LINE__);
		return NGX_ERROR;
	}

	for(loop = 0; loop < ctx->ssi_server_nodefault_sum; ++loop)
	{
		if(0 == memcmp(ctx->ssi_map[loop]->key.data, uri->data, ctx->ssi_map[loop]->key.len))
		{
			//已经找到碎片所在的机器
			if(ctx->ssi_map[loop]->value.len == 0)
			{
				memcpy(buf_4_subrequest, uri->data, uri->len);
				ctx->ssi_map[loop]->value.data = buf_4_subrequest;
				ctx->ssi_map[loop]->value.len = uri->len;
			}
			else
			{
				memcpy(buf_4_subrequest, ctx->ssi_map[loop]->value.data, ctx->ssi_map[loop]->value.len);
				memcpy(buf_4_subrequest + ctx->ssi_map[loop]->value.len, ",", 1);
				memcpy(buf_4_subrequest + ctx->ssi_map[loop]->value.len + 1, uri->data, uri->len);
				ctx->ssi_map[loop]->value.data = buf_4_subrequest;
				ctx->ssi_map[loop]->value.len = ctx->ssi_map[loop]->value.len + uri->len + 1;
			}
			fragment_in_default = 0;
			ctx->ssi_content_map_temp[loop]->value[ctx->ssi_content_map_temp[loop]->count++] = in;
			break;
		}
	}
	
	if(fragment_in_default && ((ctx->ssi_server_sum - ctx->ssi_server_nodefault_sum) != 1))
	{
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, 
			"fragment(%s) mismatched ssi_router_keys, and ssi_router_default is not found at %s:%d\n", 
			 uri->data, __FILE__, __LINE__);
		return NGX_ERROR;
	}
	else if(fragment_in_default && ((ctx->ssi_server_sum - ctx->ssi_server_nodefault_sum) == 1))
	{
		if(ctx->ssi_map[ctx->ssi_server_sum - 1]->value.len == 0)
		{
			memcpy(buf_4_subrequest, uri->data, uri->len);
			ctx->ssi_map[ctx->ssi_server_sum - 1]->value.data = buf_4_subrequest;
			ctx->ssi_map[ctx->ssi_server_sum - 1]->value.len = uri->len;
		}
		else
		{
			memcpy(buf_4_subrequest, ctx->ssi_map[ctx->ssi_server_sum - 1]->value.data, ctx->ssi_map[ctx->ssi_server_sum - 1]->value.len);
			memcpy(buf_4_subrequest + ctx->ssi_map[ctx->ssi_server_sum - 1]->value.len, ",", 1);
			memcpy(buf_4_subrequest + ctx->ssi_map[ctx->ssi_server_sum - 1]->value.len + 1, uri->data, uri->len);
			ctx->ssi_map[ctx->ssi_server_sum - 1]->value.data = buf_4_subrequest;
			ctx->ssi_map[ctx->ssi_server_sum - 1]->value.len = ctx->ssi_map[ctx->ssi_server_sum - 1]->value.len + uri->len + 1;
		}
		ctx->ssi_content_map_temp[loop]->value[ctx->ssi_content_map_temp[loop]->count++] = in;
	}
...
ngx_http_ssi_create_loc_conf(ngx_conf_t *cf)
{
	...
	slcf->router_keys = NGX_CONF_UNSET_PTR;

	slcf->optimize = NGX_CONF_UNSET;
...
ngx_http_ssi_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
	...
	ngx_conf_merge_ptr_value(conf->router_keys, prev->router_keys, NULL);
	ngx_conf_merge_str_value(conf->router_default, prev->router_default, "");
	ngx_conf_merge_value(conf->enable, prev->enable, 0);
